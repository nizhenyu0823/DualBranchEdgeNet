...
def make_wavelet_pyramid(img, level):
    current = img
    pyr = []
    for _ in range(level):
        # 使用小波变换执行滤波和下采样
        cA, (cH, cV, cD) = pywt.dwt2(current.cpu().numpy(), 'haar', mode='reflect')

        # 将 NumPy 数组转换为 PyTorch 张量
        cA = torch.from_numpy(cA).cuda()
        cH = torch.from_numpy(cH).cuda()
        cV = torch.from_numpy(cV).cuda()
        cD = torch.from_numpy(cD).cuda()
        # 将高频细节系数 (cH, cV, cD) 拼接并添加到金字塔
        diff = cD
        pyr.append(diff)
