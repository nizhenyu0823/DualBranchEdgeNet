...
from Models.lib.modules import *
from Models.quadtree import PyramidVisionTransformerV2
...
class FCBFormer(nn.Module):
    def __init__(self, size=352):
        ...
        self.gatedFusion = gatedFusion(64)
        ...
        self.down = nn.Conv2d(64, 1, kernel_size=1)
        self.ega = EGA(64)
    ...
    def forward(self, x):
        ...
        x1 = bchw_to_bhwc(x1)
        x2 = bchw_to_bhwc(x2)
        x = self.gatedFusion(x1, x2)
        # x = torch.cat((x1, x2), dim=1)#原本的相加
        x = bhwc_to_bchw(x)
        x = self.conv2(x)
        out = self.PH(x)
